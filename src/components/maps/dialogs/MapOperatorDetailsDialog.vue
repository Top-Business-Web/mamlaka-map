...
const companiesTableData = ref<LocationItem[] | []>([]);
...
watch(
  ...
  (isActive) => {
    if (isActive && locationId.value) {
      ...
      counts.value = {};
      tableData.value = [];
      companiesTableData.value = [];
    }
  }
);

watch(locationTab, (tabIdx) => {
  tableData.value = [];
  companiesTableData.value = [];
...
const getLocationDetails = async () => {
  ...
  try {
    ...
    if (locationId.value) {
      if (locationTab.value === 1) {
        getObserversListData();
        return;
      }

      if (locationTab.value === 2) {
        getCompaniesListData();
        return;
      }
    }
...
const getObserversListData = async () => {
  ...
  try {
    const { data } = await http.get(`mobility/map/observers-list?operator_id=${locationId.value}`);
...
const getCompaniesListData = async () => {
  if (!locationInfo.value && !locationInfo.value.id) return;
  isLoadingTableData.value = true;
  companiesTableData.value = [];

  try {
    const { data } = await http.get(
      `mobility/map/entities-list?operator_id=${locationInfo.value.id}`
    );
    companiesTableData.value = data?.data || [];
  } catch (err) {
    console.error(err);
  } finally {
    isLoadingTableData.value = false;
    ...
    >
      <v-card class="h-100 custom-card-variant">
        <v-card-title class="d-flex align-center justify-space-between text-h6 py-3">
          <h2 class="text-h6 font-weight-bold">معلومات المشرف</h2>
