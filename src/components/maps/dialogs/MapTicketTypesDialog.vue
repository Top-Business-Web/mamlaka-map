...
import { useMapStore, type MapInstance, type LocationType } from "@/stores/MapStore";
    ...
    >
      <v-card class="h-100 custom-card-variant">
        <v-card-title class="d-flex align-center justify-space-between text-h6 py-3">
          <div v-if="ticketType === 'high'" class="d-flex align-center ga-6">
            <v-skeleton-loader
              ...
              max-height="100%"
            ></v-skeleton-loader>
            <v-chip v-else label color="#FF7D7D" variant="flat" class="px-6">
              <strong v-if="counts && counts.total_reports != undefined" class="font-weight-bold">{{
                counts.total_reports
              }}</strong>
              <strong v-else class="font-weight-bold">0</strong>
            </v-chip>
            <h2 class="text-h6 font-weight-bold">احصائيات البلاغات ذات الاهمية العالية</h2>
          </div>
          <div v-if="ticketType === 'low'" class="d-flex align-center ga-6">
            <v-skeleton-loader
              v-if="isLoadingTableData"
              type="button"
              height="1rem"
              ...
              max-height="100%"
            ></v-skeleton-loader>
            <v-chip v-else label color="#6BA86D" variant="flat" class="px-6">
              <strong v-if="counts && counts.total_reports != undefined" class="font-weight-bold">{{
                counts.total_reports
              }}</strong>
              <strong v-else class="font-weight-bold">0</strong>
            </v-chip>
            <h2 class="text-h6 font-weight-bold">احصائيات البلاغات ذات الاهمية المتوسطة</h2>
          </div>
